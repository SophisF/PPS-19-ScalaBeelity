...
object StatisticalData {
  ...
  case class StatisticalData(
    ...
    def temperatureRange: List[Range] = {
      this expression CharacteristicTaxonomy.TEMPERATURE_COMPATIBILITY map toTuple
    }

    def pressureRange: List[Range] = {
      this expression CharacteristicTaxonomy.PRESSURE_COMPATIBILITY map toTuple
    }

    def humidityRange: List[Range] = {
      this expression CharacteristicTaxonomy.HUMIDITY_COMPATIBILITY map toTuple
    }

    def averageAggression: List[Int] = {
      this expression CharacteristicTaxonomy.AGGRESSION_RATE map toInt
    }

    def averageReproduction: List[Int] = {
      this expression CharacteristicTaxonomy.REPRODUCTION_RATE map toInt
    }

    def averageSpeed: List[Int] = {
      this expression CharacteristicTaxonomy.SPEED map toInt
    }

    def averageLongevity: List[Int] = {
      this expression CharacteristicTaxonomy.LONGEVITY map toInt
    }

    private def expression(characteristicTaxonomy: CharacteristicTaxonomy): List[Characteristic#Expression] = {
      colonies.map(c => EvolutionManager.calculateAveragePhenotype(List(c.queen) ++ c.bees).expressionOf(characteristicTaxonomy))
    }
