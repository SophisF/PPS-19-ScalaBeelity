...
import scala.model.environment.property.{BehaviouralProperty, Property, PropertyType, TimeDependentProperty}
...
object ClimateManager {
  ...
  def generateSeason(): Iterator[PropertySource[TimeDependentProperty]] = properties(_.isInstanceOf[BehaviouralProperty])
    .map(_.asInstanceOf[PropertyValue[BehaviouralProperty]]).map(seasonalChanges(_)).iterator
  ...
  private def seasonalChanges(property: BehaviouralProperty): SeasonalSource[TimeDependentProperty] =
