...
import scala.model.environment.time.Timed.{inProgress, isEnded}
...
object FiniteData {
  ...
  def dataAtInstant[R, T <: FiniteData[R]](data: T)(implicit operation: T => R): Option[R] =
    Option.when(inProgress(data, Time.time) || (data.evaluated < 100 && isEnded(data, Time.time)))(operation(data))
}